...
resource "aws_subnet" "private-subnet-1a" {
  ...
  availability_zone       = "${data.aws_region.current.name}a"
  map_public_ip_on_launch = var.map_public_ip_subnet_priv
  tags = {
...
resource "aws_route_table_association" "association-priv-rtb-1a" {
  ...
  route_table_id = var.map_public_ip_subnet_priv == true ? aws_route_table.eks-pub-rtb.id : aws_route_table.eks-priv-rtb.id
}
